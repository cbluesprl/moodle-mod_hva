...
class backup_custommailing_activity_structure_step extends backup_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $hva= new backup_nested_element('hva', array('id'), array(
            'course', 'name', 'intro', 'introformat', 'timecreated', 'timemodified'));

        $hva_trackings = new backup_nested_element('hva_trackings');

        $hva_tracking = new backup_nested_element('hva_tracking', array('id'), array(
        ...
        // Define sources
        $hva->set_source_table('hva', array('id' => backup::VAR_ACTIVITYID));

        // All the rest of elements only happen if we are including user info
        if ($userinfo) {
            $hva_tracking->set_source_table('hva_tracking', array('hva_tracking' => '../../id'));
        }
